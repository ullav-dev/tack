...
export default function Navigator() {
  ...
  useEffect(() => {
    ...
    listNoteFolders(token, activeTeam.id)
      .then((folders) => setSearchFolders(folders))
      .catch(() => {
          ...
          <div className="py-2">
            ...
            {searchResults && (searchResults.notes.total > 0 || searchResults.pages.total > 0) && (
              <>
                ...
                <Pager page={notesPage} totalPages={Math.ceil(searchResults.notes.total / SEARCH_PAGE_SIZE)} onChange={handleNotesPageChange} disabled={searching} />

                <div className="px-3 pt-3 pb-1 flex items-baseline gap-1.5">
                  <span className="text-xs font-semibold uppercase tracking-wide text-slate-400">{t("searchPagesSection")}</span>
                ...
                <Pager page={pagesPage} totalPages={Math.ceil(searchResults.pages.total / SEARCH_PAGE_SIZE)} onChange={handlePagesPageChange} disabled={searching} />
              </>
            )}
          </div>
        ) : activeTab === "notes" ? (
