...
Search (Navigator's search box, backed by `tack-server`'s hybrid `GET /search`) is grouped by content type — Notes / Pages, each its own paginated section (`Pager.tsx`) — not a single flat, unlabeled list. A hit shows its real title, a highlighted matched snippet (OpenSearch's own `<em>` fragments, parsed and rendered as a `<mark>` rather than trusted as raw HTML via `dangerouslySetInnerHTML`), and a folder badge when a note hit is filed (resolved client-side against the active team's own folder list). A reply hit links to its parent thread (`/notes/{parent_id}`), not itself, and shows "In a reply" in place of its own (always-empty) title.

Search is scoped to the active team, same as the Notes tab — `search()` takes a required `teamId`, matching `listNotes`'s own required team param. This replaced an earlier version that searched across every team you belonged to (a hit from an unrelated team could surface with no visible reason why, which read as a visibility bug even though the underlying per-note ACL was always correctly enforced). The debounced search effect re-runs on `activeTeam` changing, not just `query`, so switching teams mid-search re-fetches against the new team rather than leaving the previous team's results on screen; typing a query with no active team shows "select a team first" instead of silently doing nothing. `GET /spaces` is a known, separate exception — it still isn't team-scoped at all — deliberately left as-is rather than folded into this fix.
