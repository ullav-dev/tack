...
export default function DamPicker({ apiBase, token, username, onSelect, onDragStart, filter }: DamPickerProps) {
  ...
  return (
    <div className="flex h-full overflow-hidden bg-white">
      ...
      {/* Resize handle */}
      <div
        className="shrink-0 w-1.5 cursor-col-resize bg-slate-200 hover:bg-rose-400 transition-colors"
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="shrink-0 px-4 py-3 border-b border-slate-200 bg-white flex items-center gap-2">
          <input
            ...
            className="flex-1 rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm focus:border-rose-500 focus:outline-none focus:ring-1 focus:ring-rose-500"
          />
          <button
            type="button"
            ...
            disabled={refreshing}
            title="Refresh"
            className="shrink-0 p-1.5 rounded-lg border border-slate-300 text-slate-500 hover:text-rose-600 hover:border-rose-400 disabled:opacity-40 transition-colors bg-white"
          >
            <svg
